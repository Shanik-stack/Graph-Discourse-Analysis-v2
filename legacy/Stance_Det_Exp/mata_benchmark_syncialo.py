...
def featurize_all(
    ...
    cache_path: str = "meta_features.npz",
...
def run_meta_with_graph_logs(
    ...
    # Predict edges
    for i, (debate_id, src_id, tgt_id, premise, hypothesis, gold) in enumerate(rows[:max_examples]):
        # feature = [nli_logits(3), stance_logits(3)]
        nli_l = nli.logits(premise, hypothesis)
        stance_l = stance.logits(premise, hypothesis)
        feats = np.concatenate([nli_l, stance_l], axis=0).astype(np.float32).reshape(1, -1)
